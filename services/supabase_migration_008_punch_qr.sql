-- =============================================================================
-- Winner Pointage — Migration 008 : pointage par QR code
--
-- Idempotente. A executer apres la migration 007.
--
-- Ajoute le parametre p_qr_token a record_attendance. Quand il est fourni :
--   - le jeton est valide cote serveur (signature, fenetre, anti-rejeu) ;
--   - le SITE est impose par le jeton, pas par le client. Scanner la borne du
--     site A ne permet donc pas de pointer sur le site B ;
--   - le geofencing reste applique. Le QR prouve la presence devant la borne,
--     il ne dispense pas d'etre dans la zone : les deux controles se cumulent.
-- =============================================================================

DROP FUNCTION IF EXISTS public.record_attendance(
    VARCHAR, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION,
    TEXT, NUMERIC, TEXT, TIMESTAMP WITH TIME ZONE);
...
DECLARE
    ...
    v_method       VARCHAR(50);
BEGIN
    -- 1. Identite, strictement issue de la session -----------------------------
    ...
    -- 4. QR code : le jeton IMPOSE le site --------------------------------------
    IF v_code IS NULL AND p_qr_token IS NOT NULL AND length(trim(p_qr_token)) > 0 THEN
        ...
        IF NOT (v_qr->>'ok')::boolean THEN
            ...
            v_detail := CASE v_code
                WHEN 'QR_EXPIRED' THEN format('Ce QR code change toutes les %s secondes et n''est plus valable.',
                                              COALESCE(v_qr->>'rotation_sec', '30'))
            ...
            END;
        ELSE
            -- Site impose par le jeton, et verifie appartenir a l'entreprise.
            SELECT * INTO v_site FROM public.geofences
             WHERE id = (v_qr->>'site_id')::uuid
    ...
    -- 5. Sans QR : site le plus proche parmi ceux autorises ---------------------
    ...
    -- 6. Geofencing : applique MEME avec un QR valide ---------------------------
    -- Le QR prouve la presence devant la borne ; il ne dispense pas d'etre
    -- physiquement dans la zone. Les deux controles se cumulent.
    ...
    -- 7. Selfie -----------------------------------------------------------------
    -- Un pointage par QR devant la borne n'exige pas de selfie : la presence
    -- physique est deja etablie par le jeton a fenetre courte.
    ...
    IF v_code IS NULL AND COALESCE(v_company.face_verification_enabled, FALSE) AND p_selfie_path IS NOT NULL THEN
    ...
    -- 8. Refus : on trace la tentative, jamais une presence ---------------------
    ...
    -- 9. Retard, calcule depuis l'horaire configure -----------------------------
...
GRANT EXECUTE ON FUNCTION public.record_attendance(
    VARCHAR, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION,
    TEXT, NUMERIC, TEXT, TIMESTAMP WITH TIME ZONE, TEXT) TO authenticated;
